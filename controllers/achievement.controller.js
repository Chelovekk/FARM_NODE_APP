...
//   api/achievementincrease
        ...
        try {
         //получение данных из тела
            ...
            const {progress_increasing} = req.body;
             ...
             const ach_progress_completed = await db.query(`SELECT progress, completed FROM user_achievement_progress WHERE user_id = $1 AND ach_id=$2`, [user_id, ach_id]);
             const goal = await db.query('SELECT goal from achievementgoals WHERE ach_id = $1', [ach_id])
             //проверка и создание данных для отправки
            if(ach_progress_completed.rows.length && ach_progress_completed.rows[0].completed) {
                res.send('achievement completed')
                return
            } else if(ach_progress_completed.rows.length){
                
                let increased = parseInt(ach_progress_completed.rows[0].progress) + parseInt(progress_increasing);
    ...
    async getAchievementProgress(req,res){        
        // try{
         //получение данных из тела
            ...
            //получение даннх в таблиц данных из таблиц 
            const ach_goal = await db.query(`SELECT pp.name, p.goal FROM achievement pp JOIN achievementgoals p ON p.ach_id=pp.id WHERE id = $1`, [ach_id]);
            ...
            // const ach_progress_goal = await db.query('SELECT pp.name, p.goal, op.progress, op.completed FROM achievement pp JOIN achievementgoals p ON p.ach_id=pp.id Join user_achievement_progress op on op.ach_id = pp.id WHERE pp.id=$1 and op.user_id = $2 ', [ach_id, user_id])
            //проверка и создание данных для отправки
            ...
            const sendInfo = {
                ach_name: ach_goal.rows[0].name,
                ach_goal: ach_goal.rows[0].goal,
    ...
    async getAllAchievementProgress(req,res){
        ...
        // const ach_goal = await db.query(`SELECT pp.* p.goal FROM achievement pp Join achievementgoals p ON p.ach_id =`);
